...
def video_stream(width=640, height=480):
    ...
    We need to start a video stream using our webcam as input. 
    Then we run each frame through our progam (face detection) 
    and create an overlay image that contains bounding box of detection(s). 
    ...
    js = Javascript('''
        ...
        function onAnimationFrame() {
            ...
            if (pendingResolve) {
                var result = "";
                if (!shutdown) {
                    captureCanvas.getContext('2d').drawImage(video, 0, 0, '''+str(width)+', '+str(height)+''');
